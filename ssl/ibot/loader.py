...
class ImageFolderMask(monai.data.PersistentDataset):
    ...
    def __getitem__(self, index):
        ...
        for img in output[0]:
            ...
            high = self.get_pred_ratio() * H * W * D
            ...
            if self.pred_shape == 'block':
                ...
                while mask_count < high:
                    ...
                    for attempt in range(10):
                        ...
                        target_vol = random.uniform(low, max_mask_patches)
                        aspect_ratio1 = math.exp(random.uniform(*self.log_aspect_ratio))
                        aspect_ratio2 = math.exp(random.uniform(*self.log_aspect_ratio))
                        h = int(round(math.cbrt(target_vol * aspect_ratio1 * aspect_ratio2)))
                        w = int(round(math.cbrt(target_vol / aspect_ratio1)))
                        d = int(round(math.cbrt(target_vol / aspect_ratio2)))
                        if w < W and h < H and d < D:
                            top = random.randint(0, H - h)
                            left = random.randint(0, W - w)
                            depth = random.randint(0, D - d)

                            num_masked = mask[top: top + h, left: left + w, depth: depth + d].sum()
