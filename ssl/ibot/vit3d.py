...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = x.view(-1, self.e)
        ### As per iBOT implementation, we comment out the following two lines (14/07/2025) ###
...
class Vision_Transformer3D(nn.Module):
    ...
    def __init__(self, 
        ...
        self.return_all_tokens = kwargs.get('return_all_tokens', False)
        ...
        self.masked_im_modeling = kwargs.get('masked_im_modeling', False)
    ...
    def prepare_tokens(self, x, mask=None):
        ...
        x = x.flatten(2).transpose(1, 2)  # (B, embed_dim, n_patches) # the thing done in patch embed
    ...
    def forward(self, x, return_attn=False, return_all_tokens=None, mask=None):
        """
        Input
        -----
        ...
        if self.masked_im_modeling:
            ...
            x = self.prepare_tokens(x, mask=mask)  # (n_samples, n_patches + 1, embed_dim)
        else:
